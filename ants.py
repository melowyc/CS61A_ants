    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container and self.ant.contained_ant == None and not insect.is_container:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.direction == -1:
            destination = self.place.extrance
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive:
            #current_place.bees=current_place.entrance.bees
            if current_place.bees and float(i)>=float(self.min_range) and float(i)<=float(self.max_range):
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        bees = place.bees
        transition = 0
        while transition < 5 and place.entrance != hive:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe:    # if insect is not watersafe
            insect.armor = 1       # set the insect's armor to arbitrary value
...
# END Problem 13
    ...
    food_cost = 7
    instances = 0
    imposter = False # Change to True to view in the GUI
    # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while current_place.exit:
                ...
                if current_place.ant and not current_place.ant.double:
                    current_place.ant.double = True
                    current_place.ant.damage *= 2
                if current_place.ant and current_place.ant.is_container :#and current_place.ant
                    if current_place.ant.contained_ant and not current_place.ant.contained_ant.double:
                        current_place.ant.contained_ant.damage *= 2
                        current_place.ant.contained_ant.double = True
            ...
            #             place.ant.damage *= 2
            #             if place.ant.is_container and not place.ant.place.ant.imposter:
            #                     place.ant.place.ant.double = True
            #                     place.ant.place.ant.damage *= 2
            #
            # backward = self.place
...
def make_scare(action, bee):
    ...
    def sca_action(colony):
        destination = bee.place.entrance
        if bee.blocked():
            bee.sting(self.place.ant)
